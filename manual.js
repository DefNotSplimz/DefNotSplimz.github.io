 ...
 * Version: 5.8 (Full Operations Matrix & Thread Parsing)
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const MANUAL_OPS = {
        'manual_mill': {
            'mill_face': { name: 'Planfræsning', vc_m: 1.0, fz_m: 1.0, hide_z: false, tool_d: true },
            'mill_side': { name: 'Kantfræsning', vc_m: 1.0, fz_m: 1.0, hide_z: false, tool_d: true },
            'mill_slot': { name: 'Sporfræsning (Fuld)', vc_m: 0.8, fz_m: 0.7, hide_z: false, tool_d: true },
            'mill_drill_hss': { name: 'Boring (HSS)', vc_m: 0.8, fz_m: 1.0, hide_z: false, tool_d: true },
            'mill_drill_hm': { name: 'Boring (HM)', vc_m: 1.0, fz_m: 1.0, hide_z: false, tool_d: true },
            'mill_ream': { name: 'Rivalisering', vc_m: 0.5, fz_m: 2.0, hide_z: true, tool_d: true },
            'mill_chamfer': { name: 'Undersænkning', vc_m: 0.5, fz_m: 0.5, hide_z: false, tool_d: true },
            'tap': { name: 'Gevindboring', vc_m: 0.3, fz_m: 1.0, hide_z: true, tool_d: true }
        },
        'manual_turn': {
            'turn_rough': { name: 'Skrub Drejning', vc_m: 1.0, fz_m: 1.0, hide_z: true, tool_d: false },
            'turn_finish': { name: 'Slet Drejning (Ra)', vc_m: 1.3, fz_m: 1.0, hide_z: true, tool_d: false },
            'turn_groove': { name: 'Afstikning / Spor', vc_m: 0.6, fz_m: 0.5, hide_z: true, tool_d: false },
            'turn_thread': { name: 'Gevindskæring', vc_m: 0.4, fz_m: 1.0, hide_z: true, tool_d: false },
            'turn_drill': { name: 'Boring', vc_m: 0.8, fz_m: 1.0, hide_z: true, tool_d: true },
            'turn_center': { name: 'Centrumsboring', vc_m: 0.8, fz_m: 0.5, hide_z: true, tool_d: true },
            'turn_ream': { name: 'Rivalisering', vc_m: 0.5, fz_m: 2.0, hide_z: true, tool_d: true },
            'tap': { name: 'Gevindboring i Centrum', vc_m: 0.3, fz_m: 1.0, hide_z: true, tool_d: true }
        }
    };
    ...
    function populateOperations() {
        ...
        labelFzOut.textContent = type === 'manual_mill' ? 'Target fz' : 'Target fn';
        outFzUnit.textContent = type === 'manual_mill' ? 'MM/Z' : 'MM/REV';
        outVfUnit.textContent = type === 'manual_mill' ? 'MM/MIN' : 'MM/REV';
        labelOutFeed.textContent = type === 'manual_mill' ? '// Bord-Tilspænding' : '// Manuel Tilspænding';
    ...
    function handleOperationChange() {
        ...
        colZ.style.display = opData.hide_z ? 'none' : 'flex';
    ...
    function calculate() {
        ...
        // Basis-reduktion for manuelle maskiner lagt sammen med specifik operation multiplier
        let vc = (toolMat === 'HM' ? mat.vc_hm : mat.vc_hss) * 0.55 * opData.vc_m;
        let fz = mat.fz_ref * opData.fz_m;
        let feedText = "";
        ...
        if (op === 'turn_thread') {
            ...
            fz = pitchVal; // I drejning er Pitch = Feed/Rev
            feedText = pitchVal.toFixed(3);
        } else {
             if (machine.type === 'manual_mill' && !opData.hide_z) {
                 feedText = Math.round(n * z * fz);
             } else {
        ...
        const zNote = (machine.type === 'manual_mill' && !opData.hide_z) ? `Z${z}` : '-';
    ...
    function calculateIso() {
        ...
        if (match && match[cls]) {
            ...
            resEl.classList.remove('text-primary');
        }
    }
        ...
        if(tableBody) {
            tableBody.innerHTML = logs.map(entry => `
                <tr class="border-b border-zinc-900/50 bg-black/20 hover:bg-primary/[0.03] transition-all tabular-nums group" data-id="${entry.id}">
                    <td class="p-6">
                        <span class="text-xl font-black italic tracking-tighter text-white">${entry.machine}</span>
                    </td>
                    <td class="p-6">
                        <div class="flex flex-col">
                            <span class="text-xs font-black text-zinc-300 uppercase italic tracking-tight">${entry.op}</span>
                            <span class="label-micro !text-[7px] !text-zinc-600 !before:hidden mt-1 uppercase print:hidden">Analog_Execution</span>
                        </div>
                    </td>
                    <td class="p-6">
                        <span class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">${entry.geom}</span>
                    </td>
                    <td class="p-6">
                        <div class="flex flex-col">
                    ...
                    <td class="p-6">
                        <div class="flex flex-col">
                            <span class="text-lg font-black italic text-white tracking-tighter">${entry.feed}</span>
                            <span class="text-[8px] font-mono text-zinc-600 font-bold tracking-widest uppercase">FEED</span>
                        </div>
                    </td>
                    <td class="p-6">
                        <span class="text-[10px] font-bold text-primary uppercase tracking-widest">${entry.note}</span>
                    </td>
    ...
    [workMaterialSelect, toolMatSelect, inputDia, inputZ, threadSize, modRa].forEach(el => {
