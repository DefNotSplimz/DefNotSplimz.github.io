'use strict';
const projects={
flame:{title:'Flame Eater Engine',index:'01 / MODELMOTOR',image:'flame-engine.webp',material:'Messing · aluminium · stål',process:'Manuel fræsning og drejning',focus:'Pasning mellem cylinder og stempel',intro:'Flame Eater Engine er det projekt, jeg er mest stolt af. Jeg har selv fremstillet alle motorens dele. Kun kuglelejerne er indkøbt.',challenge:'Den største udfordring var at få pasningen mellem cylinder og stempel rigtig. Her mødes kravene til bevægelse og tæthed i to dele, der skal fungere sammen.',solution:'Jeg brugte en kombination af GO/NO-GO-lærer og løbende afprøvning af pasningen mellem cylinder og stempel. Motoren kører tørt uden smøring, så jeg foretog løbende justeringer af pasningen. Jeg kontrollerede både målene med lærerne og afprøvede, hvordan cylinder og stempel passede sammen undervejs. De tre FAI-rapporter dokumenterer kontrollen af cylinder og stempel, knast og svinghjul.',learning:'At kontrollere de kritiske mål undervejs og tænke pasninger ind som en del af konstruktionen.',docs:[['FAI 01','Cylinder og stempel','fai-01.html'],['FAI 02','Cam Disc / knast','fai-02-camdisc.html'],['FAI 03','Svinghjul','fai-03-flywheel.html']]},
ring:{title:'Ring Bender',index:'02 / SPECIALVÆRKTØJ',image:'ring-bender.webp',material:'Aluminium 6082 · stål',process:'CNC-fræsning · Haas Mini Mill',focus:'Stivhed under bukning',intro:'En manuel ringbøjer, hvor en fleksibel FDM-konstruktion er gentænkt som et stivere værktøj i metal.',challenge:'Det tidligere FDM-værktøj gav for meget efter under bukning. Konstruktionen skulle kunne optage belastningen mere stabilt.',solution:'Bearbejdet aluminium og stål erstatter den fleksible konstruktion. De belastede flader og de enkelte deles samspil er centrale for løsningen.',learning:'At materialevalg og stivhed er afgørende, når en CAD-model skal fungere som værktøj i praksis.',docs:[]},
...
document.querySelector('#project-content').innerHTML=`<figure class="dialog-visual"><img src="images/${p.image}" alt="Visualisering af ${p.title}" width="1536" height="1024"><figcaption>AI-visualisering baseret på min CAD-rendering.</figcaption></figure><div class="dialog-copy"><h2 id="project-title">${p.title}</h2><p class="dialog-intro">${p.intro}</p><dl class="project-facts"><div><dt>Materiale</dt><dd>${p.material}</dd></div><div><dt>Proces</dt><dd>${p.process}</dd></div><div><dt>Fokus</dt><dd>${p.focus}</dd></div></dl><div class="project-story"><section><h3>Udfordringen</h3><p>${p.challenge}</p></section><section><h3>Løsningen</h3><p>${p.solution}</p></section></div><section class="project-learning"><h3>Det tager jeg med videre</h3><p>${p.learning}</p></section>${p.docs.length?`<section class="project-documents"><h3>Dokumentation</h3><p>${p.docs.length===3?'Tre FAI-rapporter med kontrol af de første fremstillede emner.':'Opspænding, værktøjsliste og operationsrækkefølge.'}</p><div class="doc-links">${p.docs.map(d=>`<a href="documents/${d[2]}"><span class="doc-kind">${d[0]}</span><strong>${d[1]}</strong><span aria-hidden="true">→</span></a>`).join('')}</div></section>`:''}</div>`;
